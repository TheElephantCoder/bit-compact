...
pub fn add(a: &[f32], b: &[f32], out: &mut [f32]) -> Result<()> {
    if a.len() != b.len() || a.len() != out.len() {
        return Err(CompactError::DimensionMismatch { expected: a.len(), found: b.len() });
    }
...
pub fn scale(a: &[f32], s: f32, out: &mut [f32]) -> Result<()> {
    if a.len() != out.len() {
        return Err(CompactError::DimensionMismatch { expected: a.len(), found: out.len() });
    }
...
pub fn variance(vectors: &[Vec<f32>], mean: &[f32]) -> Result<Vec<f32>> {
    ...
    for v in vectors {
        if v.len() != dims { return Err(CompactError::DimensionMismatch{ expected: dims, found: v.len()}); }
        for d in 0..dims { let diff = v[d]-mean[d]; var[d]+=diff*diff; }
    }
    for x in &mut var { *x/=vectors.len() as f32; }
    Ok(var)
}

pub fn clamp(v: &mut [f32], lo: f32, hi: f32) {
    for x in v { if *x < lo { *x = lo; } else if *x > hi { *x = hi; } }
}
...
mod tests {
    ...
    #[test]
    fn add_sub() {
        let a=[1.0,2.0]; let b=[3.0,4.0]; let mut out=[0.0;2];
        add(&a,&b,&mut out).unwrap(); assert_eq!(out,[4.0,6.0]);
        sub(&b,&a,&mut out).unwrap(); assert_eq!(out,[2.0,2.0]);
    }
    #[test]
    fn norm() { assert!((l2_norm(&[3.0,4.0]) -5.0).abs()<1e-6); }
}
