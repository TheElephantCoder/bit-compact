use bit_compact::{Quantizer, distance};

fn main() {
    // Cheap bench without criterion (zero deps). Run with `cargo run --release --bench quant_bench`
    let dims = 128;
    let n = 10_000;
    let data: Vec<Vec<f32>> = (0..n).map(|i| (0..dims).map(|d| ((i+d)%100) as f32 / 50.0 -1.0).collect()).collect();
    ...
    println!("quantize {n}x{dims}: {:?} ({:.0} vec/s) sink {sink}", elapsed, n as f64 / elapsed.as_secs_f64());
