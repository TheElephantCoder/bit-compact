...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let dataset: Vec<Vec<f32>> = (0..count)
        .map(|i| (0..dims).map(|d| ((i * 7 + d * 13) % 100) as f32 / 50.0 - 1.0).collect())
        .collect();
    ...
    for hit in &top3p { println!("  id={} dist={:.4}", hit.id, hit.distance); }
    ...
    println!("cos-like search top2 for [1;8]: {:?}", cos.iter().map(|r| r.id).collect::<Vec<_>>());
